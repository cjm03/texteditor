...
void editorSave(void)
{
    ...
    /* we tell open to create a new file if DNE, and open it for R/W (O_RDWR). */
    int fd = open(E.filename, O_RDWR | O_CREAT, 0644); // 0644 - standard text file perms (R/W for owner, R for others)
    if (fd != -1) {
